...
export default function App() {
  ...
  // ── Play count tracking: increment + update local state + history ──
  useEffect(() => {
    player.onPlayStart = async (song) => {
      await recordPlay(song.id);
      // Update local song state so UI reflects new play count
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const allSongs = await getAllSongs();
    setSongs(allSongs);
    showToast(`Added ${result.added} song${result.added !== 1 ? 's' : ''}`);
    e.target.value = '';
  };
  ...
  const handleRescanFolder = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const allSongs = await getAllSongs();
    setSongs(allSongs);
    showToast(`Added ${result.added} new song${result.added !== 1 ? 's' : ''}${skipped > 0 ? ` (${skipped} already in library)` : ''}`);
    e.target.value = '';
  };
          ...
          <div className="flex-1 min-w-0 flex flex-col overflow-hidden">
            ...
            <div className="flex items-center gap-2 px-3 md:px-4 pt-3 pb-2 shrink-0" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
              ...
              <div className={`${isSpecialView ? 'ml-auto' : ''} flex items-center gap-1`}>
                ...
                <label className="btn-icon w-9 h-9 hover:bg-white/8 cursor-pointer shrink-0" title="Rescan folder for new songs">
                  <RefreshCw size={16} className="text-white/50" />
                  <input type="file" ref={rescanInputRef}
                    // @ts-ignore
            ...
            {/* ── STATS VIEW ── */}
            {view === 'stats' ? (
              <StatsScreen songs={songs} history={history} accentColor={accentColor} onClearHistory={handleClearHistory} onPlaySong={handlePlay} listeningStats={listeningStats.stats} />
            ) : view === 'queue' ? (
              /* ── QUEUE VIEW ── */
              <QueuePanel
                queue={upcomingSongs}
